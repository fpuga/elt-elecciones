...
@asset(deps=[zips_to_postgres], group_name="elecciones", compute_kind="Transform")
def transform_candidatos(context: AssetExecutionContext) -> None:
    schema = "prod"
    table = "candidatos"
    with psycopg2.connect(
        "postgresql://postgres:postgres@localhost:5432/elecciones"
    ) as conn:
        with conn.cursor() as curs:
            ...
            query = f"""
                ...
                CREATE TABLE {schema}.{table} (
                    ...
                    , mes int not null
                    , vuelta int not null
                    , ine_provincia int not null
                    , distrito_electoral int not null
                    , ine_municipio int not null
                    , codigo_candidatura int not null
                    , orden_candidato int not null
                    , tipo_candidato text not null
                    , nombre text not null
                    , sexo text not null
                    -- , fecha_nacimiento date not null Es siempre 0
                    , dni text
            ...
            curs.execute(query)
            query = """
                INSERT INTO prod.candidatos
                    (tipo, mes, vuelta, ine_provincia, distrito_electoral, ine_municipio, codigo_candidatura, orden_candidato, tipo_candidato, nombre, sexo, dni, elegido)
                SELECT
                     tipo, mes, vuelta, ine_provincia, distrito_electoral, ine_municipio, codigo_candidatura, orden_candidato, tipo_candidato
                     , trim(nombre) || ' ' || trim(coalesce(primer_apellido, '')) || ' ' || trim(coalesce(segundo_apellido, ''))
                     , sexo, dni
                     , case elegido when 'S' then true when 'N' then false else null end
                FROM raw.candidatos;
            """
            curs.execute(query)
